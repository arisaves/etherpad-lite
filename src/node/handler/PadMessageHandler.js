...
exports.handleMessage = async (socket, message) => {
  ...
  if (!auth) {
    console.error(`Auth was never applied to a session.
      If you are using the stress-test tool then restart
      Etherpad and the Stress test tool.`);
    return;
  }
...
const handleGetChatMessages = async (socket, message) => {
  ...
  if (count < 0 || count > 100) {
    messageLogger.warn(`Dropped message, GetChatMessages Message, client requested
      invalid amount of messages!`);
    return;
  }
      ...
      while (iterator.hasNext()) {
        ...
        op.attribs.split('*').forEach((attr) => {
          ...
          if ('author' === attr[0] && (attr[1] !== thisSession.author && attr[1] !== '')) {
            throw new Error(`Author ${thisSession.author} tried to submit changeset
                as author ${attr[1]} in changeset ${changeset}`);
          }
        });
      }
    ...
    while (r < pad.getHeadRevisionNumber()) {
      ...
      try {
        ...
        if (baseRev + 1 === r && c === changeset) {
          socket.json.send({disconnect: 'badChangeset'});
          stats.meter('failedChangesets').mark();
          throw new Error('Wont apply USER_CHANGES, as it contains an already accepted changeset');
        }
...
const _correctMarkersInPad = (atext, apool) => {
  ...
  while (iter.hasNext()) {
    ...
    const hasMarker = _.find(
        ...
        (attribute) => Changeset.opAttributeValue(op, attribute, apool)
    ) !== undefined;
...
const handleClientReady = async (socket, message, authorID) => {
  ...
  if (message.protocolVersion !== 2) {
    messageLogger.warn(`Dropped message, CLIENT_READY Message has a unknown
...
const handleChangesetRequest = async (socket, message) => {
  ...
  try {
    ...
    console.error(`Error while handling a changeset request for
        ${padIds.padId}`, err.toString(), message.data);
  }
};
